...
from src.utils.common import read_yaml, get_first_row_from_dataset, \
    get_runs_model_names_from_dir, get_hg_model_names_from_dir, get_hg_model_names_and_gguf_from_dir, \
    validate_model_path, get_runs_models
...
import socket
from config import setting_value as vl_st
...
def get_local_dataset_list():
    ...
    matched_dataset_file_path_list = glob.glob(os.path.join(local_dataset_root_dir, "**", "dataset_infos.json"),
                                               recursive=False)
    ...
    matched_dataset_file_path_list = glob.glob(os.path.join(local_dataset_root_dir, "**", "dataset_dict.json"),
                                               recursive=False)
    for matched_file_path in matched_dataset_file_path_list:
        matched_pos1 = matched_file_path.rfind("datasets")
...
with gr.Blocks(title="FINETUNE",
               css="#vertical_center_align_markdown { position:absolute; top:30%;background-color:white;} .white_background {background-color: #ffffff} .none_border {border: none;border-collapse:collapse;}") as demo:
    init()
    ...
    local_embedding_model_root_dir_textbox = gr.Textbox(label="",
                                                        value=os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                                           "rag", "embedding_models"), visible=False)
    ...
    with gr.Tab("Home"):
        with gr.Row():
            ...
            with gr.Group():
                ...
                with gr.Row():
                    chatbot = gr.Chatbot(value=[], bubble_full_width=False, rtl=False, layout="panel",
                                         height=chatbot_height,
                                         avatar_images=((os.path.join(os.path.abspath(''), "pics", "user1.png")),
                                                        (os.path.join(os.path.abspath(''), "pics", "bot4.png"))),
                                         )
                with gr.Row():
                        ...
                        with gr.Group():
                            ...
                            with gr.Group():
                                ...
                                with gr.Group():
                                    ...
                                    if validate_model_path(training_base_model_names[0])[0]:
                                        download_model_status_markdown = gr.Markdown(
                                    ...
                                    with gr.Row():
                                        ...
                                        local_model_list = get_hg_model_names_from_dir(
                                            os.path.dirname(os.path.abspath(__file__)), "models")
                                        local_model_dropdown = gr.Dropdown(local_model_list, label="Local Model",
                                                                           info="",
                                                                           value=local_model_list[0] if len(
                                                                               local_model_list) > 0 else None,
                                                                           interactive=True,
                            ...
                            with gr.Group():
                                ...
                                with gr.Row(equal_height=True):
                                    hg_dataset_path_textbox = gr.Textbox(label="Dataset Name:",
                                                                         elem_classes="none_border", visible=False,
                                ...
                                with gr.Row():
                                    hg_train_dataset_dropdown = gr.Dropdown(["train"], label="Train set", info="",
                                                                            interactive=False, visible=False,
                                                                            elem_classes="white_background", scale=1,
                                                                            value="train")
                                    hg_val_dataset_dropdown = gr.Dropdown([], label="Val set", info="",
                                                                          interactive=False, visible=False,
                                                                          elem_classes="white_background", scale=1)
                                ...
                                with gr.Row():
                                    ...
                                    local_train_path_dataset_dropdown = gr.Dropdown(local_dataset_list,
                                                                                    label="Train Dataset", info="",
                                                                                    value=local_dataset_list[0] if len(
                                                                                        local_dataset_list) > 0 else None,
                                                                                    interactive=True,
                                                                                    elem_classes="white_background",
                                                                                    scale=5, visible=True)
                                    refresh_local_train_path_dataset_list_btn = gr.Button("Refresh", scale=1,
                                                                                          visible=True)
                                with gr.Row():
                                    local_train_dataset_dropdown = gr.Dropdown(["train"], label="Train set", info="",
                                                                               interactive=True,
                                                                               elem_classes="white_background", scale=1,
                                                                               value="train", visible=True)
                                    local_val_dataset_dropdown = gr.Dropdown([], label="Val set", info="",
                                                                             interactive=True,
                                                                             elem_classes="white_background", scale=1,
                                                                             visible=True)
                                ...
                                with gr.Group(elem_classes="white_background"):
                                    ...
                                    gr.Markdown(
                                        "<br> &nbsp;&nbsp;&nbsp;&nbsp;**Prompt Template: (Prefix1+ColumnName1+Prefix2+ColumnName2+Prefix3+ColumnName3+Prefix4+ColumnName4)**",
                                        elem_classes="white_background")
                                    gr.Markdown(
                                    ...
                                    with gr.Row(elem_classes="white_background"):
                                        # prompt_template
                                        prefix1_textbox = gr.Textbox(label="Prefix1:", value=INIT_PREFIX1, lines=2,
                                                                     interactive=True, elem_classes="white_background")
                                        datatset_col1_dropdown = gr.Dropdown(col_names, label="ColumnName1:", info="",
                                                                             value=col_names[1], interactive=True,
                                                                             elem_classes="white_background")
                                        prefix2_textbox = gr.Textbox(label="Prefix2:", value=INIT_PREFIX2, lines=2,
                                                                     interactive=True, elem_classes="white_background")
                                        datatset_col2_dropdown = gr.Dropdown(col_names, label="ColumnName2:", info="",
                                                                             value=col_names[2], interactive=True,
                                                                             elem_classes="white_background")
                                    with gr.Row(elem_classes="white_background"):
                                        prefix3_textbox = gr.Textbox(label="Prefix3:", value=INIT_PREFIX3, lines=2,
                                                                     interactive=True, elem_classes="white_background")
                                        datatset_col3_dropdown = gr.Dropdown(col_names, label="ColumnName3:", info="",
                                                                             value=col_names[3], interactive=True,
                                                                             elem_classes="white_background")
                                        prefix4_textbox = gr.Textbox(label="Prefix4:", value=INIT_PREFIX4, lines=2,
                                                                     interactive=True, elem_classes="white_background")
                                        datatset_col4_dropdown = gr.Dropdown(col_names, label="ColumnName4:", info="",
                                                                             value=col_names[0], interactive=True,
                                                                             elem_classes="white_background")
                                    # print("")
                                    ...
                                    prompt_sample_textbox = gr.Textbox(label="Prompt Sample:", interactive=False,
                                                                       value=prompt_sample, lines=4)
                            ...
                            with gr.Group():
                                ...
                                with gr.Row(elem_classes="white_background"):
                                    optimizer_dropdown = gr.Dropdown(transformer_optimizer_list, label="Optimizer",
                                                                     ...
                                                                     interactive=True)
                                    lr_scheduler_list = ["linear", "cosine", "cosine_with_hard_restarts",
                                                         "polynomial_decay", "constant", "constant_with_warmup",
                                ...
                                with gr.Row(elem_classes="white_background"):
                                    eval_steps_slider = gr.Slider(0, 1000, value=100, step=100, label="eval_steps",
                                                                  interactive=True)
                                    gradient_checkpointing_checkbox = gr.Checkbox(True, label="Gradient Checkpointing",
                                                                                  interactive=True)
                    ...
                    with gr.Column(scale=1, min_width=1):
                        with gr.Group():
                            ...
                            with gr.Row():
                                ...
                                finetune_test_using_4bit_quantization_checkbox = gr.Checkbox(True,
                        ...
                        with gr.Group():
                            ...
                            with gr.Row():
                                ...
                                quantization_type_dropdown = gr.Dropdown(quantization_type_list,
                                                                         label="Quantization Type",
                                                                         value=quantization_type_list[0],
                                                                         interactive=True, scale=3)
                                local_quantization_dataset_dropdown = gr.Dropdown(local_dataset_list,
                                                                                  label="Dataset for quantization",
                                                                                  value=local_dataset_list[0] if len(
                                                                                      local_dataset_list) > 0 else None,
                                                                                  interactive=True,
                                                                                  elem_classes="white_background",
                                ...
                                refresh_local_quantization_dataset_btn.click(
                                    click_refresh_local_quantization_dataset_btn, [],
                            ...
                            if runs_output_model:
                                ...
                                quantized_model_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'runs',
                                                                   os.sep.join(
                                                                       runs_output_model[0].split(os.sep)[0:-1]),
                                                                   "quantized_" + quantization_type_list[
                                                                       0] + "_" + model_name)
                                if not os.path.exists(quantized_model_dir):
                                    os.makedirs(quantized_model_dir)
                                quantization_logging_markdown = gr.Markdown("")
                                gguf_quantization_markdown0 = gr.Markdown(
                                    "### &nbsp;&nbsp;&nbsp;&nbsp;GGUF Quantization Instruction:",
                                    elem_classes="white_background", visible=True)
                                gguf_quantization_markdown1 = gr.Markdown(
                                    '''&nbsp;&nbsp;&nbsp;&nbsp;1.Follow the instructions in the llama.cpp to generate a GGUF:[https://github.com/ggerganov/llama.cpp#prepare-data--run](https://github.com/ggerganov/llama.cpp#prepare-data--run),<span style="color:red">&nbsp;&nbsp;Q4_K_M is recommend</span>''',
                                    visible=True)
                                if runs_output_model:
                                    gguf_quantization_markdown2 = gr.Markdown(
                                        f"&nbsp;&nbsp;&nbsp;&nbsp;2.Convert {runs_output_model[0]} to gguf model",
                                        visible=True)
                                else:
                                    gguf_quantization_markdown2 = gr.Markdown(
                                        f"", visible=True)
                                gguf_quantization_markdown3 = gr.Markdown(
                                    f"&nbsp;&nbsp;&nbsp;&nbsp;3.Deploy gguf model", visible=False)
                            else:
                        ...
                        with gr.Group(visible=False):
                            ...
                            with gr.Row():
                                deployment_framework_dropdown = gr.Dropdown(["TGI", "llama-cpp-python"],
                                                                            label="Deployment Framework", value="TGI",
                                                                            interactive=True)
                            with gr.Row():
                                training_runs_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'runs')
                                run_names = os.listdir(training_runs_dir)
                                ...
                                deployment_runs_output_model_dropdown = gr.Dropdown(tgi_model_format_runs_output_model,
                                                                                    label="runs_output_model",
                                                                                    value=
                                                                                    tgi_model_format_runs_output_model[
                            ...
                            if tgi_model_format_runs_output_model:
                                ...
                                run_server_script_textbox = gr.Textbox(label="Run Server:", interactive=False, lines=2,
                                                                       scale=1, value=run_server_value)
                                ...
                                run_client_script_textbox = gr.Textbox(label="Run Client:", interactive=False, lines=6,
                                                                       scale=1, value=run_client_value)
                            else:
                                run_server_script_textbox = gr.Textbox(label="Run Server:", interactive=False, lines=2,
                                                                       scale=1, value="")
                                run_client_script_textbox = gr.Textbox(label="Run Client:", interactive=False, lines=6,
  ...
  && \
    sudo apt-get update </p>
                            ...
                            run_llama_cpp_python_code = gr.Code("", language="python", lines=10,
                                                                label="run_model_using_llama_cpp_python.py",
                                                                visible=False)
            ...
            with gr.TabItem("Tensorboard", id=1) as fdddd:
                ...
                with gr.Row():
                    ...
                    with gr.Group():
                        # with gr.Row():
                        training_runs_dropdown = gr.Dropdown(run_names, label="Training Runs",
        ...
        with gr.Row():
            with gr.Column(scale=4, min_width=1):
                with gr.Group():
                    ...
                    #                                    interactive=True)
                    with gr.Row():
                        rag_data_list_dropdown = gr.Dropdown(matched_file_name_list,
                                                             label=f"Local Documents In {rag_data_dir}",
                                                             value=matched_file_name_list[
                                                                 0] if matched_file_name_list else None,
                                                             interactive=True, scale=4, min_width=1)
                    ...
                    def click_refresh_rag_data_list_btn():
                        ...
                        for matched_file in matched_file_list:
                            matched_file_name_list.append(os.path.basename(matched_file))
                        return gr.update(choices=matched_file_name_list,
                    ...
                    with gr.Row():
                        rag_chatbot = gr.Chatbot(value=[], bubble_full_width=False, rtl=False, layout="panel",
                                                 height=chatbot_height,
                                                 avatar_images=(
                                                     (os.path.join(os.path.abspath(''), "pics", "user1.png")),
                                                     (os.path.join(os.path.abspath(''), "pics", "bot4.png"))),
                                                 )
                    with gr.Row():
                ...
                with gr.Group():
                    ...
                    with gr.Group():
                        ...
                        with gr.Group():
                            ...
                            local_embedding_model_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "rag",
                                                                     "embedding_models")
                            local_embedding_model_names = get_hg_model_names_from_dir(local_embedding_model_dir,
                                                                                      "embedding_models")
                            ...
                            with gr.Row():
                                hub_embedding_model_names_dropdown = gr.Dropdown(embedding_model_names,
                                                                                 ...
                                                                                 value=embedding_model_names[
                                                                                     0] if embedding_model_names else None,
                                                                                 interactive=True, scale=4, min_width=1)
                                ...
                                local_embedding_model_names_dropdown = gr.Dropdown(local_embedding_model_names,
                                                                                   ...
                                                                                   show_label=False,
                                                                                   value=local_embedding_model_names[
                                                                                       0] if local_embedding_model_names else None,
                                                                                   interactive=True, scale=4,
                                                                                   min_width=1, visible=False)
                            ...
                            if os.path.exists(model_config_path):
                                ...
                                download_hub_embedding_model_status_markdown = gr.Markdown(
                                    '<span style="color:red">&nbsp;&nbsp;&nbsp;&nbsp;This model has not been downloaded.</span>')
                            with gr.Row():
                                search_top_k_slider = gr.Slider(1, 10, value=3, step=1, label="Search Top K",
                                                                interactive=True)
                                search_score_threshold_slider = gr.Slider(0, 1, value=0.5, step=0.1,
                        ...
                        with gr.Group():
                            ...
                            local_chat_model_names = get_hg_model_names_and_gguf_from_dir(local_chat_model_dir,
                                                                                          runs_model_root_dir)
                            ...
                            with gr.Row():
                                ...
                                local_chat_model_names_dropdown = gr.Dropdown(local_chat_model_names,
                                                                              label=f"Chat Model", show_label=False,
                                                                              value=local_chat_model_names[
                                                                                  0] if local_chat_model_names else None,
                                                                              interactive=True, scale=4, min_width=1,
                                                                              visible=False)
                                refresh_local_chat_model_names_btn = gr.Button("Refresh", scale=1, visible=False)
                                rag_using_4bit_quantization_checkbox = gr.Checkbox(True,
                                                                                   label="Using 4-bit quantization",
                                                                                   interactive=True, visible=True,
                                                                                   info="Less memory but slower",
                                                                                   scale=1
    ...
    with gr.Tab("Setting"):
        ...
        with gr.Group():
            ...
            with gr.Group():
                with gr.Row():
                    max_new_tokens_slider = gr.Slider(1, 4096, value=vl_st.max_new_tokens, step=0.1,
                                                      label="Max New Tokens",
                                                      interactive=True)
                    temperature_slider = gr.Slider(0, 5, value=vl_st.temperature, step=0.1, label="Temperature",
                                                   interactive=True)
                with gr.Row():
                    top_k_slider = gr.Slider(1, 100, value=vl_st.Top_k, step=1, label="Top_k",
                                             interactive=True)
                    top_p_slider = gr.Slider(0, 1, value=vl_st.Top_p, step=0.1, label="Top_p",
                                             interactive=True)
                with gr.Row():
                    repeat_penalty_slider = gr.Slider(1, 5, value=vl_st.Repeat_Penalty, step=0.1,
                                                      label="Repeat Penalty", interactive=True)
                with gr.Row():
                    chat_history_window_slider = gr.Slider(1, 20, value=vl_st.Chat_History_Window, step=1,
                                                           label="Chat History Window",
                                                           interactive=True)
                    low_cpu_mem_usage_checkbox = gr.Checkbox(False, label="Low Cpu Mem Usage", interactive=True,
                                                             visible=False)
    ...
    def change_refresh_local_home_chat_model_names_btn():
        local_home_chat_model_names = get_hg_model_names_and_gguf_from_dir(local_home_chat_model_dir,
                                                                           runs_model_root_dir)
        return gr.update(choices=local_home_chat_model_names,
                         value=local_home_chat_model_names[0] if local_home_chat_model_names else None)


    refresh_local_home_chat_model_names_btn.click(change_refresh_local_home_chat_model_names_btn, [],
                                                  [local_home_chat_model_names_dropdown])
    ...
    def click_load_home_chat_model_btn(home_chat_model_source_radio, hub_home_chat_model_names_dropdown,
                                       ...
                                       chat_history_window_slider, using_4bit_quantization_checkbox,
    ...
    def change_dataset_source(dataset_source_radio, base_model_source_radio, base_model_name_dropdown,
                              local_model_dropdown, hg_dataset_path_textbox):
        ...
        if dataset_source_radio == "Download From Huggingface Hub":
            ...
            if not curr_model_name or model_type == "other model":
                return gr.update(choices=local_dataset_list,
                                 value=local_dataset_list[0] if local_dataset_list else None), \
                    gr.update(choices=split_list, value=split_list[0] if split_list else None), \
                    gr.update(choices=split_list, value=None), \
                    gr.update(visible=False), gr.update(visible=False), gr.update(visible=True), gr.update(
                    visible=True), \
                    gr.update(visible=True), gr.update(visible=True), gr.update(visible=False), gr.update(
                    visible=False), \
                ...
                if "instruction" in col_names and "input" in col_names and "output" in col_names:
                    ...
                    return gr.update(choices=local_dataset_list,
                        ...
                        visible=False), \
                        gr.update(visible=False), gr.update(visible=False), \
                        gr.update(value=INIT_PREFIX1, visible=True), gr.update(choices=col_names, value=col_names[
                        col_names.index("instruction")]), \
                        gr.update(value=INIT_PREFIX2, visible=True), gr.update(choices=col_names, value=col_names[
                        col_names.index("input")]), \
                        gr.update(value=INIT_PREFIX3, visible=True), gr.update(choices=col_names, value=col_names[
                        col_names.index("output")]), \
                        gr.update(value=INIT_PREFIX4, visible=True), gr.update(choices=col_names, value="")
                else:
                    if model_type == "mistral" or model_type == "llama2":
    ...
    def change_dataset_col1(prefix1_textbox, datatset_col1_dropdown, prefix2_textbox, datatset_col2_dropdown,
    ...
    def change_dataset_col2(prefix1_textbox, datatset_col1_dropdown, prefix2_textbox, datatset_col2_dropdown,
                            prefix3_textbox, datatset_col3_dropdown, prefix4_textbox, datatset_col4_dropdown):
        prompt = prefix1_textbox
        if isinstance(datatset_col1_dropdown, str) and datatset_col1_dropdown in DATASET_FIRST_ROW:
            prompt += str(DATASET_FIRST_ROW[datatset_col1_dropdown])
    ...
    def change_dataset_col3(prefix1_textbox, datatset_col1_dropdown, prefix2_textbox, datatset_col2_dropdown,
                            prefix3_textbox, datatset_col3_dropdown, prefix4_textbox, datatset_col4_dropdown):
        prompt = prefix1_textbox
        if isinstance(datatset_col1_dropdown, str) and datatset_col1_dropdown in DATASET_FIRST_ROW:
            prompt += str(DATASET_FIRST_ROW[datatset_col1_dropdown])
    ...
    def change_dataset_col4(prefix1_textbox, datatset_col1_dropdown, prefix2_textbox, datatset_col2_dropdown,
                            prefix3_textbox, datatset_col3_dropdown, prefix4_textbox, datatset_col4_dropdown):
        prompt = prefix1_textbox
        if isinstance(datatset_col1_dropdown, str) and datatset_col1_dropdown in DATASET_FIRST_ROW:
            prompt += str(DATASET_FIRST_ROW[datatset_col1_dropdown])
    ...
    def change_prefix1_textbox(prefix1_textbox, datatset_col1_dropdown, prefix2_textbox, datatset_col2_dropdown,
                               prefix3_textbox, datatset_col3_dropdown, prefix4_textbox, datatset_col4_dropdown):
        prompt = prefix1_textbox
        if isinstance(datatset_col1_dropdown, str) and datatset_col1_dropdown in DATASET_FIRST_ROW:
            prompt += str(DATASET_FIRST_ROW[datatset_col1_dropdown])
    ...
    def change_prefix2_textbox(prefix1_textbox, datatset_col1_dropdown, prefix2_textbox, datatset_col2_dropdown,
                               prefix3_textbox, datatset_col3_dropdown, prefix4_textbox, datatset_col4_dropdown):
        prompt = prefix1_textbox
        if isinstance(datatset_col1_dropdown, str) and datatset_col1_dropdown in DATASET_FIRST_ROW:
            prompt += str(DATASET_FIRST_ROW[datatset_col1_dropdown])
    ...
    def change_prefix3_textbox(prefix1_textbox, datatset_col1_dropdown, prefix2_textbox, datatset_col2_dropdown,
                               prefix3_textbox, datatset_col3_dropdown, prefix4_textbox, datatset_col4_dropdown):
        prompt = prefix1_textbox
        if isinstance(datatset_col1_dropdown, str) and datatset_col1_dropdown in DATASET_FIRST_ROW:
            prompt += str(DATASET_FIRST_ROW[datatset_col1_dropdown])
    ...
    def change_prefix4_textbox(prefix1_textbox, datatset_col1_dropdown, prefix2_textbox, datatset_col2_dropdown,
                               prefix3_textbox, datatset_col3_dropdown, prefix4_textbox, datatset_col4_dropdown):
        prompt = prefix1_textbox
        if isinstance(datatset_col1_dropdown, str) and datatset_col1_dropdown in DATASET_FIRST_ROW:
            prompt += str(DATASET_FIRST_ROW[datatset_col1_dropdown])
    ...
    def change_base_model_source(base_model_source_radio, base_model_name_dropdown, local_model_dropdown,
        ...
        if base_model_source_radio == "Download From Huggingface Hub":
            ...
            return gr.update(visible=True), gr.update(visible=False), gr.update(visible=False), gr.update(
                visible=True), gr.update(value=""), gr.update(value=model_download_status), gr.update(
                visible=True), gr.update(visible=False), gr.update(visible=True), gr.update(visible=True), gr.update(
            ...
            return gr.update(visible=False), gr.update(visible=True), gr.update(visible=True), gr.update(
                visible=False), gr.update(value=""), gr.update(value=model_download_status), gr.update(
                visible=False), gr.update(visible=False), INIT_PREFIX1, INIT_PREFIX2, INIT_PREFIX3, INIT_PREFIX4
    ...
    def change_base_model_name_dropdown(base_model_name_dropdown):
        ...
        if not base_model_name_dropdown:
            return "", "", "", "", gr.update(visible=True,
                                             value='<span style="color:red">&nbsp;&nbsp;&nbsp;&nbsp;No model is selected.</span>'), gr.update(
                visible=True), gr.update(visible=False)
        ...
        if validate_model_path(base_model_name_dropdown)[0]:
            return INIT_PREFIX1, INIT_PREFIX2, INIT_PREFIX3, INIT_PREFIX4, gr.update(visible=True,
                                                                                     value='<span style="color:green">&nbsp;&nbsp;&nbsp;&nbsp;This model has already been downloaded to local.</span>'), gr.update(
                visible=True), gr.update(visible=False)
        else:
            return INIT_PREFIX1, INIT_PREFIX2, INIT_PREFIX3, INIT_PREFIX4, gr.update(visible=True,
                                                                                     value='<span style="color:red">&nbsp;&nbsp;&nbsp;&nbsp;This model has not been downloaded.</span>'), gr.update(
                visible=True), gr.update(visible=False)


    def change_local_train_path_dataset_dropdown(local_train_path_dataset_dropdown, base_model_name_dropdown,
                                                 base_model_source_radio, local_model_dropdown):
        global DATASET_FIRST_ROW
        local_dataset_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'datasets',
                                          local_train_path_dataset_dropdown)
        DATASET_FIRST_ROW, split_list = get_first_row_from_dataset(local_dataset_path)
        ...
        if not curr_model_name or model_type == "other model":
            ...
            if "instruction" in col_names and "input" in col_names and "output" in col_names:
                ...
                return gr.update(choices=split_list, value=split_list[0] if split_list else None), gr.update(
                    choices=split_list, value=None), \
                    INIT_PREFIX1, gr.update(choices=col_names,
                                            value=col_names[col_names.index("instruction")]), INIT_PREFIX2, gr.update(
                    choices=col_names,
                    value=col_names[col_names.index("input")]), \
                    INIT_PREFIX3, gr.update(choices=col_names,
                                            value=col_names[col_names.index("output")]), INIT_PREFIX4, gr.update(
                    choices=col_names, value="")
            else:
                if model_type == "mistral" or model_type == "llama2":
                    INIT_PREFIX1 = "<s>[INST] "
                    INIT_PREFIX2 = " [/INST]"
                ...
                return gr.update(choices=split_list, value=split_list[0] if split_list else None), gr.update(
                    choices=split_list, value=None), \
                    INIT_PREFIX1, gr.update(choices=col_names, value=INIT_COL1_TEXT), INIT_PREFIX2, gr.update(
                    choices=col_names, value=INIT_COL2_TEXT), \
                    INIT_PREFIX3, gr.update(choices=col_names, value=INIT_COL3_TEXT), INIT_PREFIX4, gr.update(
                    choices=col_names, value=INIT_COL4_TEXT)


    local_train_path_dataset_dropdown.change(change_local_train_path_dataset_dropdown,
                                             [local_train_path_dataset_dropdown, base_model_name_dropdown,
                                              base_model_source_radio, local_model_dropdown],
                                             [local_train_dataset_dropdown, local_val_dataset_dropdown, prefix1_textbox,
                                              datatset_col1_dropdown, prefix2_textbox, datatset_col2_dropdown,
                                              prefix3_textbox,
    ...
    dataset_source_radio.change(fn=change_dataset_source,
                                inputs=[dataset_source_radio, base_model_source_radio, base_model_name_dropdown,
                                        local_model_dropdown, hg_dataset_path_textbox],
                                outputs=[local_train_path_dataset_dropdown, local_train_dataset_dropdown,
                                         local_val_dataset_dropdown,
                                         hg_dataset_path_textbox, download_local_dataset_btn,
                                         local_train_path_dataset_dropdown,
                                         refresh_local_train_path_dataset_list_btn, local_train_dataset_dropdown,
                                         local_val_dataset_dropdown, download_dataset_status_markdown,
                                         stop_download_local_dataset_btn,
                                         hg_train_dataset_dropdown, hg_val_dataset_dropdown, prefix1_textbox,
                                         datatset_col1_dropdown, prefix2_textbox, datatset_col2_dropdown,
                                         prefix3_textbox,
                                         datatset_col3_dropdown, prefix4_textbox, datatset_col4_dropdown
                                         ])
    datatset_col1_dropdown.change(change_dataset_col1,
                                  [prefix1_textbox, datatset_col1_dropdown, prefix2_textbox, datatset_col2_dropdown
                                      , prefix3_textbox, datatset_col3_dropdown, prefix4_textbox,
                                   datatset_col4_dropdown], [prompt_sample_textbox])
    datatset_col2_dropdown.change(change_dataset_col2,
                                  [prefix1_textbox, datatset_col1_dropdown, prefix2_textbox, datatset_col2_dropdown
                                      , prefix3_textbox, datatset_col3_dropdown, prefix4_textbox,
                                   datatset_col4_dropdown], [prompt_sample_textbox])
    datatset_col3_dropdown.change(change_dataset_col3,
                                  [prefix1_textbox, datatset_col1_dropdown, prefix2_textbox, datatset_col2_dropdown
                                      , prefix3_textbox, datatset_col3_dropdown, prefix4_textbox,
                                   datatset_col4_dropdown], [prompt_sample_textbox])
    datatset_col4_dropdown.change(change_dataset_col4,
                                  [prefix1_textbox, datatset_col1_dropdown, prefix2_textbox, datatset_col2_dropdown
                                      , prefix3_textbox, datatset_col3_dropdown, prefix4_textbox,
                                   datatset_col4_dropdown], [prompt_sample_textbox])
    ...
    click_download_local_model_btn_event = download_local_model_btn.click(check_local_model_or_dataset_is_empty2,
                                                                          [base_model_name_dropdown,
                                                                           Huggingface_hub_token]).success(
        click_download_local_model_btn, [],
        [download_local_model_btn,
         stop_download_local_model_btn,
         download_model_status_markdown]).then(
        download_model_wrapper, [base_model_name_dropdown, local_model_root_dir_textbox],
        download_model_status_markdown) \
    ...
    stop_download_local_model_btn.click(click_stop_download_local_model_btn, [],
                                        [download_local_model_btn, stop_download_local_model_btn,
                                         download_model_status_markdown],
                                        cancels=[click_download_local_model_btn_event])

    base_model_source_radio.change(change_base_model_source,
                                   [base_model_source_radio, base_model_name_dropdown, local_model_dropdown,
                                    local_train_path_dataset_dropdown],
                                   [base_model_name_dropdown, local_model_dropdown, refresh_local_model_list_btn,
                                    download_model_status_markdown, download_model_status_markdown,
                                    download_model_status_markdown, download_local_model_btn,
                                    stop_download_local_model_btn,
    ...
    def check_training_params(prefix3_textbox, datatset_col3_dropdown, prefix4_textbox, datatset_col4_dropdown,
                              dataset_source_radio, hg_dataset_path_textbox, local_train_path_dataset_dropdown,
                              local_train_dataset_dropdown, local_val_dataset_dropdown,
                              prefix1_textbox, datatset_col1_dropdown, prefix2_textbox, datatset_col2_dropdown,
                              max_length_dropdown,
                              base_model_source_radio, local_model_dropdown, base_model_name_dropdown,
                              fine_tuning_type_dropdown,
                              lora_r_slider, lora_alpha_slider, lora_dropout_slider, lora_bias_dropdown, epochs_slider,
                              batch_size_slider,
                              learning_rate_slider, optimizer_dropdown, lr_scheduler_type_dropdown,
                              gradient_checkpointing_checkbox, gradient_accumulation_steps_slider,
                              warmup_steps_slider, early_stopping_patience_slider, eval_steps_slider,
                              hg_train_dataset_dropdown, hg_val_dataset_dropdown):
        ...
        if dataset_source_radio == "Download From Huggingface Hub":
            ...
            if not os.path.exists(os.path.join(dataset_dir, "dataset_infos.json")) and not os.path.exists(
    ...
    def click_train_btn(prefix3_textbox, datatset_col3_dropdown, prefix4_textbox, datatset_col4_dropdown,
                        ...
                        gradient_checkpointing_checkbox, gradient_accumulation_steps_slider,
                        warmup_steps_slider, early_stopping_patience_slider, eval_steps_slider,
                        hg_train_dataset_dropdown, hg_val_dataset_dropdown, training_log_progress=gr.Progress()):
        ...
        if train_param_config["dataset"]["is_huggingface_dataset"]:
            train_param_config["dataset"]["hg_dataset_dir"] = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                                           "datasets", train_param_config["dataset"][
                                                                               "huggingface_dataset_name"])
        else:
            train_param_config["dataset"]["hg_dataset_dir"] = None
            train_param_config["dataset"]["local_dataset_dir"] = os.path.join(
        ...
        train_param_config["model"]["base_model_path"] = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                ...
                if qlora_trainer.logging_callback.max_steps > 0:
                    if last_step >= 0:
                        ...
                        if elapsed_steps >= 5 and start_time >= 0:
                            ...
                            remaining_time = (
                                                     qlora_trainer.logging_callback.max_steps - qlora_trainer.logging_callback.current_step) * time_per_step
        ...
        if training_ret_val == 0:
            ...
            yield f"### &nbsp;&nbsp;<span style='color:green'>Training process is over: {qlora_trainer.logging_callback.current_step} /{qlora_trainer.logging_callback.max_steps}</span>", gr.update(
                choices=runs_model_names, value=runs_model_names[0] if runs_model_names else None), gr.update(
                choices=runs_model_names, value=runs_model_names[0] if runs_model_names else None)
        else:
            TRAINING_STATUS.status = 2
            qlora_trainer.free_memroy()
            runs_model_names = get_runs_models()
            yield f"### &nbsp;&nbsp;<span style='color:red'>Training is interrupted: {error_msg}</span>", gr.update(
                choices=runs_model_names,
                value=runs_model_names[0] if runs_model_names else None), gr.update(
                choices=runs_model_names, value=runs_model_names[0] if runs_model_names else None)
    ...
    train_params = [prefix3_textbox, datatset_col3_dropdown, prefix4_textbox, datatset_col4_dropdown,
    ...
    train_btn.click(update_test_input_prompt,
                    [prefix1_textbox, datatset_col1_dropdown, prefix2_textbox, datatset_col2_dropdown,
                     prefix3_textbox, datatset_col3_dropdown, prefix4_textbox, datatset_col4_dropdown],
                    test_input_textbox). \
        then(check_training_params, train_params).success(change_to_tensorboard_tab, None,
                                                          [tensorboard_tab, tensorboard_html]).then(click_train_btn,
                                                                                                    train_params,
                                                                                                    [
                                                                                                        training_log_markdown,
                                                                                                        runs_output_model_dropdown,
                                                                                                        quantization_runs_output_model_dropdown])
    refresh_local_model_list_btn.click(click_refresh_local_model_list_btn, [refresh_local_model_list_btn],
                                       local_model_dropdown)
        ...
        if runs_output_model_dropdown:
            if test_input_textbox.strip():
                ...
                if output_model_path.split('.')[-1] == "gguf":
                    ...
                    infer_model = HuggingfaceInference(model_path=output_model_path,
                                                       max_new_tokens=max_new_tokens_slider,
                                                       temperature=temperature_slider,
                                                       top_k=top_k_slider, top_p=top_p_slider,
                                                       repetition_penalty=repeat_penalty_slider,
    ...
    generate_text_btn.click(click_generate_text_btn,
                            [runs_output_model_dropdown, test_input_textbox, max_new_tokens_slider,
                             temperature_slider, top_k_slider, top_p_slider, repeat_penalty_slider,
                             chat_history_window_slider, finetune_test_using_4bit_quantization_checkbox,
                             low_cpu_mem_usage_checkbox], test_output)
    ...
    def change_quantization_runs_output_model_dropdown(quantization_runs_output_model_dropdown,
                                                       quantization_type_dropdown):
        # model_name = quantization_runs_output_model_dropdown.split(os.sep)[-2].split('_')[-1]
        # quantized_model_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'runs',
        #                                    os.sep.join(quantization_runs_output_model_dropdown.split(os.sep)[0:-1]),
        #                                    "quantized_" + quantization_type_dropdown + "_" + model_name)
        ...
        #     os.makedirs(quantized_model_dir)
        return gr.update(
            value=f"&nbsp;&nbsp;&nbsp;&nbsp;2.Convert {quantization_runs_output_model_dropdown} to gguf model")
    ...
    def click_refresh_deployment_runs_output_model_btn1(deployment_framework_dropdown):
        ...
        if deployment_framework_dropdown == "TGI":
            ...
            return gr.update(choices=gguf_model_format_runs_output_model,
                             value=gguf_model_format_runs_output_model[
                                 0] if gguf_model_format_runs_output_model else "")
    ...
    def change_deployment_runs_output_model_dropdown(deployment_framework_dropdown,
                                                     deployment_runs_output_model_dropdown):
        if deployment_framework_dropdown == "TGI":
    ...
    refresh_local_embedding_model_names_btn.click(click_refresh_local_embedding_model_names_btn, [],
                                                  local_embedding_model_names_dropdown)
    ...
    refresh_local_chat_model_names_btn.click(click_refresh_local_chat_model_names_btn, [],
                                             local_chat_model_names_dropdown)
    ...
    def rag_chat_with_model1(input_txtbox):
        ...
        yield chatbot_history, gr.update(
            ...
            value=None), f"### &nbsp;&nbsp;Retrieved Document Chunks({len(retrieved_txt_list)} items)", gr.update(
            value="")
    ...
    rag_input_txtbox.submit(check_rag_config1, check_rag_config_params, rag_model_running_status_markdown). \
        ...
        success(rag_chat_with_model1, inputs=[rag_input_txtbox],
                outputs=[rag_chatbot, retrieved_document_chunks_dataframe, rag_model_running_status_markdown,
                         rag_input_txtbox])
